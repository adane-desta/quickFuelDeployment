...
export function FuelPriceManagement() {
  ...
  const [editValues, setEditValues] = useState<{ pricePerLiter: number; effectiveFrom: string }>({
    pricePerLiter: 0,
    effectiveFrom: '',
  });
  ...
  const handleCancel = () => {
    setEditingId(null);
    setEditValues({ pricePerLiter: 0, effectiveFrom: '' });
  };

  const handleSave = async (priceId: string) => {
    if (editValues.pricePerLiter <= 0) {
      toast.error('Price must be greater than 0');
      return;
    }

    if (!editValues.effectiveFrom) {
    ...
    try {
      const updates = {
        pricePerLiter: editValues.pricePerLiter,
      ...
      const fuelType = fuelPrices.find((p) => p.id === priceId)?.fuelType;
      toast.success(`${fuelType} price updated successfully`, {
        description: `New price: ETB ${editValues.pricePerLiter.toFixed(2)}/L effective from ${new Date(editValues.effectiveFrom).toLocaleDateString()}`,
      });
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {fuelPrices.map((price) => {
              ...
              const change = getChangePercentage(price.pricePerLiter);
              const isIncrease = parseFloat(change) > 0;

              return (
                <Card key={price.id} className="overflow-hidden">
                  <div className={`p-4 border-b ${price.fuelType === 'Petrol' ? 'bg-blue-50 border-blue-200' : 'bg-green-50 border-green-200'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                            price.fuelType === 'Petrol' ? 'bg-blue-500' : 'bg-green-500'
                          }`}
                        >
                          <DollarSign className="w-6 h-6 text-white" />
                        </div>
                        <div>
                          <h3 className="text-gray-900">{price.fuelType}</h3>
                    ...
                    {isEditing ? (
                      <div className="space-y-4">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setEditValues((prev) => ({ ...prev, pricePerLiter: parseFloat(e.target.value) || 0 }))}
                            className="mt-2"
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setEditValues((prev) => ({ ...prev, effectiveFrom: e.target.value }))}
                            className="mt-2"
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="text-center py-4 bg-gray-50 rounded-xl">
                          <p className="text-4xl text-gray-900 mb-1">ETB {price.pricePerLiter.toFixed(2)}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-3 bg-gray-50 rounded-lg">
                            ...
                            <p className="text-sm text-gray-900">{new Date(price.updatedAt).toLocaleDateString()}</p>
                            <p className="text-xs text-gray-500 mt-1">{new Date(price.updatedAt).toLocaleTimeString()}</p>
                          </div>
                          <div className="p-3 bg-gray-50 rounded-lg">
                            ...
                            <p className="text-sm text-gray-900">{new Date(price.effectiveFrom).toLocaleDateString()}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg border border-green-200">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-900">{price.updatedBy}</p>
                          </div>
                        </div>
                        ...
                        <div className="border-t pt-4">
                          ...
                          <div className="space-y-1 text-sm text-gray-600">
                            <div className="flex justify-between">
                              <span>20 liters:</span>
                              <span className="text-gray-900">ETB {(price.pricePerLiter * 20).toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>50 liters:</span>
                              <span className="text-gray-900">ETB {(price.pricePerLiter * 50).toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>100 liters:</span>
                              <span className="text-gray-900">ETB {(price.pricePerLiter * 100).toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
